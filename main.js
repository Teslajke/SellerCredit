jQuery(function ($) {
    ...
    function num2str(t, a) {
        ...
        return t > 10 && t < 20 ? a[2] : e > 1 && e < 5 ? a[1] : 1 == e ? a[0] : a[2]
    }
    ...
    $("#email").on('focusout', function () {
        /^[a-z0-9][a-z0-9-_.]{0,}[a-z0-9]@[a-z0-9][a-z0-9-_.]{0,}[a-z0-9]\.[a-z0-9][a-z0-9-_.]{0,}[a-z0-9]$/i.test($("#email").val()) ? ($(".ermail").removeClass("activer"), o = !0) : ($(".ermail").addClass("activer"), o = !1)
    });

    $('#amount').on('input', function() {
        $(this).prev('.range-value').text($(this).val().replace(/(\d)(?=(\d{3})+(\D|$))/g, "$1 "));
        
    })
    $('#period').on('input', function() {
        $(this).prev('.range-value').text($(this).val() + ' ' + num2str(parseInt($(this).val()), ["месяц", "месяца", "месяцев"]));
    })

    $("#fee").on('focusin',function() {
        $(this).val(parseInt($(this).val()))
    }).on('focusout', function() {
        let t = $(this).val();
        t = t.replace(/\s/g, ""),
        "" == t || isNaN(parseInt(t)) ? $(this).val("0%") : $(this).val(parseInt(t) + "%");
    });
